...
from backend_main import Game, play_move
import pprint
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                ...
                if len(playerClicks) == 2 and move_no%2 == 0:
                    move =GUI_helper.Move(playerClicks[0], playerClicks[1], gs.board)
                    ...
                    try:
                        ...
                        for square in board_dict:
                            boardList[GUI_helper.Move.filesToCols[square[0].lower()]][GUI_helper.Move.ranksToRows[square[1]]] = board_dict[square]
                ...
                p.display.set_caption('AutoChess - Computer\'s turn  (Loading....)')
                ...
                gs.board =[[row[i] for row in boardList] for i in range(len(boardList[0]))]
                pp.pprint(gs.board)
                p.display.set_caption('AutoChess - Your turn')
                playsound("Assets/sounds/move_sound.mp3")
                move_no = move_no + 1
